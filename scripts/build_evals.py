...
def render(h2a, a2a, disc):
    ...
    for layer in sorted(by):
        ...
        for r in rows[:3]:                                  # a few representative cases per group
            ...
            h2a_html += (f'<tr><td>{e(str(r.get("input"))[:70])}</td>'
                         f'<td class="mono">{e(r.get("attack_class"))}</td>'
                         f'<td>{e(exp)}</td><td class="where">{e(r["scorer"])}</td><td>{st}</td></tr>')
        if len(rows) > 3: h2a_html += f'<tr><td colspan="5" class="mono">… +{len(rows)-3} more in this layer</td></tr>'
