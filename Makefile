.PHONY: setup check lint showcase evals evals-braintrust gate run demo demo-all deploy preflight hf smoke bakeoff harness mcp
...
showcase:  ; python -m presentation.build_all      # generate the 12-stop showcase + print the completion matrix
lint:      ; ruff check . && mypy agent api evals tools presentation   # style (ruff) + basic type bugs (mypy); black NOT run (compact style)
