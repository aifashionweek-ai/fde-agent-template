...
| 2:35–3:10 | **(e) Open vs closed** | "Same governed agent, three models — one closed (Claude), two open on Bedrock in-account (Qwen3, Llama3.3). Every deterministic *safety* scorer is 1.00 across all three. The governance makes the models interchangeable on the things that must not fail; the only spread is nuanced quality. A residency-bound customer runs open weights with zero safety loss." | `cat docs/model-eval-2026-08-19.md` |
...
| 3:45–4:00 | **Close** | "Governance is model-independent, every layer is a tested invariant, and `make check` is 58 green. The same pattern runs in production." | back to the terminal: `make check` line |
