...
| **0:30–0:50** | **SLOT 1 · Golden set** | `evals/dataset.jsonl` — write 5–8 cases incl. an adversarial slice (injection / cross-tenant / out-of-policy). Add per-layer rows in `evals/datasets/` if the domain needs new invariants. | The golden set encodes the spec; `make evals` runs (may fail — that's the target). |
...
| **1:10–1:55** | **SLOT 3 · Tools + schema** | Edit the tool table in `MASTERSCHEMA.md`; implement `@tool` fns in `agent/tools.py` (keep read vs action shape + `authorize()` before acting); `python update.py` to regen + drift-check. | `make check` green with the domain tools; action tools hit the approval gate. |
