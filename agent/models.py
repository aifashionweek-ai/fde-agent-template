...
REGISTRY: dict[str, ModelProfile] = {p.id: p for p in [
    ...
    # --- open weights SERVED BY BEDROCK (serverless, per-token, in customer account). IDs vary by region/date:
    #     verify with `scripts/bedrock_preflight.sh` / `aws bedrock list-foundation-models` and set BEDROCK_*_ID envs.
    ModelProfile("deepseek-v3.2-bedrock", "bedrock", os.getenv("BEDROCK_DEEPSEEK_ID", "deepseek.v3.2"),
                 "open", "customer_vpc", cost_tier=2, latency_tier=3, quality_tier=4,
                 good_for=("reasoning","codegen","summarization"), license="mit", notes="reasoning/agentic; verify id"),
    ModelProfile("qwen3-coder-bedrock", "bedrock", os.getenv("BEDROCK_QWEN_ID", "qwen.qwen3-coder-next"),
                 "open", "customer_vpc", cost_tier=2, latency_tier=2, quality_tier=4,
                 good_for=("codegen","extraction"), license="apache-2.0", notes="verify id"),
    ModelProfile("kimi-k2.5-bedrock", "bedrock", os.getenv("BEDROCK_KIMI_ID", "moonshotai.kimi-k2.5"),
                 "open", "customer_vpc", cost_tier=2, latency_tier=3, quality_tier=4,
                 good_for=("reasoning","summarization"), license="modified-mit", notes="verify id"),
    ModelProfile("ministral-8b-bedrock", "bedrock", os.getenv("BEDROCK_MINISTRAL_ID", "mistral.ministral-3-8b-instruct"),
                 "open", "customer_vpc", cost_tier=1, latency_tier=1, quality_tier=2,
                 good_for=("classification","extraction"), license="apache-2.0", notes="cheap router/classifier; verify id"),
    ModelProfile("gpt-oss-20b-bedrock", "bedrock", os.getenv("BEDROCK_GPTOSS_ID", "openai.gpt-oss-20b"),
                 "open", "customer_vpc", cost_tier=1, latency_tier=1, quality_tier=3,
                 good_for=("classification","extraction","summarization"), license="apache-2.0", notes="verify id"),
